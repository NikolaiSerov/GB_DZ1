...
bool FindNumbers(int[,] inArray, int Number, int rowIndex, int colmIndex)
{
    rowIndex = -1;
    rowIndex = -1;
    for (int i = 0; i < inArray.GetLength(0); i++)
...
PrintArray(array);
if (FindNumbers(array, number, rows, columns))
{
    Console.WriteLine($"Индексы искомого числа равны: [{rows}, {columns}]");
}
else
{
